...
namespace Orlen.Services
{
    ...
    public abstract class WebService
    {
        ...
        public WebService(string token, string userId, string clientID) : this()
        {
            _token = token;
            _userID = userId;
            _clientID = clientID;
        ...
        protected async Task<ListType> SendRequestAsyncWithDataResultList<ListType>(
                    Uri url, HttpMethod httpMethod = null,
                    IDictionary<string, string> headers = null
                   )
        {
                ...
                if (response.IsSuccessStatusCode && response.Content != null)
                {
                    ...
                    result = JsonConvert.DeserializeObject<ListType>(content);
                }
                return result;
            //}
            //catch (Exception ex)
